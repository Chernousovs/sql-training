...
`create index movies_release_date_idx on ${MOVIES} (
  release_date
)`;

const CREATE_INDEX_MOVIE_RATINGS_TIME_CREATED = 
`create index movie_ratings_time_created_idx on ${MOVIE_RATINGS} (
  time_created
)`;

const CREATE_UNIQUE_INDEX_MOVIES_IMDB_ID = 
`create unique index movies_imdb_id_unq_idx on ${MOVIES} (
  imdb_id
)`;

const CREATE_UNIQUE_INDEX_KEYWORDS_KEYWORD = 
`create unique index keywords_keyword_unq_idx on ${KEYWORDS} (
 keyword
)`;

const CREATE_UNIQUE_INDEX_GENRES_GENRE = 
`create unique index genres_genre_unq_idx on ${GENRES}(
  genre
)`;
